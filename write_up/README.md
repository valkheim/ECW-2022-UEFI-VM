This challenge is an EFI app asking for a password/flag, bundled in disk.img file.
...
This output should give you somes hints about what's inside.
...
Here, I show the decompression / decoding process for the static way:

Locate the second stage (help yourself from xrefs):

![Second stage](second_stage.png)

Extract compressed/encoded second stage app:
...
I used the python-eficompressor package which is a python wrapper for the standard decompression algorithm described in the UEFI specification. (the lzma one, not the tiano custom one).
...
At this point we can rot/grep the efi app. I'll do it on the data section specifically:
